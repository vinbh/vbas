...
#
# The install puts:
#   binary  -> $HOME/.local/bin/peek  (override: PREFIX=/usr/local)
#   hook    -> $HOME/.config/peek/peek.zsh
#   specs   -> $HOME/.config/peek/specs/
#
...
echo "==> installing peek $TAG for ${OS}/${ARCH}"

ARCHIVE="peek_${OS}_${ARCH}.tar.gz"
...
EXTRACTED="$TMPDIR/peek_${OS}_${ARCH}"
if [[ ! -d "$EXTRACTED" ]]; then
...
echo ""
echo "Add this to your ~/.zshrc to enable peek:"
echo "    source \$HOME/.config/peek/peek.zsh"
